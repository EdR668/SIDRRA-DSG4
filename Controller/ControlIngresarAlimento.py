...
from Model.DB import insert_document, organicos_colletion, inorganicos_collection, create_alimento
import uuid
    ...
    def obtain_data(self):
        ...
        if selection == "Organico":
            ...
            if self.confirm(cant=cant,nombre=nombre,tipo=tipo,comb=selection,caducidad=caducidad):
                new_food = Organico(uuid.uuid1().hex,cant,nombre,tipo,uso,caract,props,save_image(self.imagen[0]),caducidad,datetime.strptime(str(datetime.today().strftime('%d-%m-%Y')),'%d-%m-%Y'))
                insert_document(organicos_colletion,create_alimento(new_food))
                self.blank_fields()
            ...
            if self.confirm(cant=cant,nombre=nombre,tipo=tipo,comb=selection,day=day,month=month,year=year):
                n_food = Inorganico(uuid.uuid1().hex,cant, nombre, tipo, uso, caract, props, save_image(self.imagen[0]), str(year)+"-"+str(month)+"-"+str(day),datetime.strptime(str(datetime.today().strftime('%d-%m-%Y')),'%d-%m-%Y'))
                insert_document(inorganicos_collection, create_alimento(n_food))
                self.blank_fields()
                self.var_day.set("")
                self.var_month.set("")
                self.var_year.set("")
